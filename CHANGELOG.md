...
- The Unreal-host C smoke probe now checks that an undeclared collision-enabled
  enum is rejected before component-handle lookup.
...
- ABI minor 132 adds a Blueprint-assignable actor event component, bounded C
  callback subscriptions, event emission from C, teardown cleanup, and the
  `UEC_CAPABILITY_EVENT_BRIDGE` feature bit. The host
...
- ABI minor 140 adds the two Slate hit-test visibility modes, allowing visible
  widgets to ignore pointer targeting while preserving or disabling child hits.
...
- The host Game/PIE probe now spawns a native latent-test actor, checks
  ...
  count and scalar-kind rejection, typed FVector/FQuat/FTransform round trips
  and mismatch rejection, short text-output sizing and retry, invalid
  ...
  entries.
- A separately compiled ABI 1.135 header consumer links against the ABI 1.140
