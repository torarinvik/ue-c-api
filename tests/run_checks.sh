...
"${CC:-cc}" -std=c11 -Wall -Wextra -Werror -pedantic-errors -I "$public_dir" \
    ...
    "$host_abi_consumer" "$host_event_consumer" \
...
for source_file in "$repo_dir"/tests/c_smoke/c_smoke_layout.c; do
    line_count=$(wc -l < "$source_file" | tr -d ' ')
    if [ "$line_count" -lt 400 ] || [ "$line_count" -gt 800 ]; then
        printf 'Smoke layout unit is outside the 400-800 line budget: %s (%s lines)\n' \
            "$source_file" "$line_count" >&2
        exit 1
    fi
done
