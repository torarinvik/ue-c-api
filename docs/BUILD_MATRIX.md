...
preflight, short text-output sizing and retry, invalid world-kind handling,
completion, cancellation, signature rejection, explicit and cross-world
context handling, and pending request counts, so Unreal Build Tool does not
need to synthesize temporary targets before compiling the plugin and its first
C consumer.
