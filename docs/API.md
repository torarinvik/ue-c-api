...
diagnostics, world, player, timer, streaming, object, save-game, path-query,
and queued-request adapters. A failed call with a non-null output pointer
therefore leaves a null handle, `UEC_FALSE`, zero, or an empty value instead of
preserving stale caller data.
...
`destroy_audio_component`, and then release the handle. The sound and
