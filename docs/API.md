...
ABI minor 119 adds `get_class_property_struct_field_count` and
...
Non-null handle or text fields that do not match the selected kind are
rejected.
...
ABI minor 141 appends `get_checkbox_state` and `set_checkbox_state` for
`UCheckBox` handles. The `uec_checkbox_state` values represent unchecked,
