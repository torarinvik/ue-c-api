...
| Reflection and Blueprint calls | Partial | Property writes, class/function/parameter/flag metadata, reflected actor/object-array counts/text elements, UObject map/set counts and text entries, zero-argument calls, scalar typed calls, bounded text-marshaled calls, scalar multi-output calls, and text multi-output calls are available; broader typed ABI calls remain |
