...
| Reflection and Blueprint calls | Partial | Property writes, class/function/parameter metadata, zero-argument calls, scalar typed calls, bounded text-marshaled calls, and scalar multi-output calls are available; strings, containers, and broader typed ABI calls remain |
