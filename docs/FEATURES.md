...
| Basic UMG widgets | Runtime implemented / Unreal integration pending | Create a `UUserWidget` class, obtain weak handles to named tree children, add or remove it from the viewport, set/read five visibility modes and enabled state, update/read `UTextBlock` text, read/write normalized `UProgressBar` percent and all three `UCheckBox` states, and receive one-shot button clicks |
