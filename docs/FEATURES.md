...
| Basic UMG widgets | Runtime implemented / Unreal integration pending | Create a `UUserWidget` class, add or remove it from the viewport, set/read visibility and enabled state, update/read `UTextBlock` text, read/write normalized `UProgressBar` percent, and receive one-shot button clicks |
