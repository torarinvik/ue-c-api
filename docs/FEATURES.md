...
| Game-thread dispatch | Runtime implemented / Unreal integration pending | Queue and cancel borrowed callbacks from worker threads; FIFO dispatch drains at most 64 callbacks per engine tick and admits 1024 pending requests; packaged probe checks concurrent admission, queue-full output clearing, unique ids, released-context rejection, successful-cancel suppression, in-callback accounting, the dequeued-cancel result, and request drainage |
