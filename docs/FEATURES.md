...
| Reflection and Blueprint calls | Partial | Property writes, class/function/parameter/flag metadata including property access flags and class-default text, reflected actor/object-array counts/text elements, typed scalar array/map/set reads and array/map writes, typed nested-struct scalar reads and writes, actor/UObject map/set counts and text entries, soft object/class path readback and writes, nested struct field text readback and writes, zero-argument calls, scalar typed calls, bounded text-marshaled calls, scalar multi-output calls, and text multi-output calls are available; broader typed ABI calls remain |
