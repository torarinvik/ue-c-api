...
| Reflection and Blueprint calls | Partial | Property writes, zero-argument calls, scalar typed calls, and bounded text-marshaled calls with one return/first out value are available; strings, containers, multiple outputs, and broader typed ABI calls remain |
