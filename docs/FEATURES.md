...
| Game-thread dispatch | Runtime implemented / Unreal integration pending | Queue and cancel borrowed callbacks from worker threads; FIFO dispatch drains at most 64 callbacks per engine tick and admits 1024 pending requests; packaged probe checks concurrent admission, queue-full output clearing, unique ids, cancellation suppression, in-callback accounting, and request drainage |
...
| Asynchronous object loading | Runtime implemented / Unreal integration pending | Streamable-manager requests, cancellation, game-thread callbacks, and a 1024-request bound; packaged host cancels a loaded-asset request and checks immediate request drainage plus callback suppression before a second request verifies object path, callback accounting, and handle release |
